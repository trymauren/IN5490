...
def make_combined_plots_bipop(runs_path):
    rc('font',**{'family':'serif','serif':['Times']})
    rc('text', usetex=True)
    filenames = []
    ...
    for filename in filenames:  
        ...
        with shelve.open(filename, 'c') as fp: 
            ...
            for i, d in enumerate(fp):
                ...
                runs_logbooks = fp[str(i)]
                
                for run_logbook in runs_logbooks:
                    temp_run += run_logbook.chapters['fitness'].select('avg')
                    temp_run_max += run_logbook.chapters['fitness'].select('max')


                gen_book.append(temp_run[:1000])
                gen_book_max.append(temp_run_max[:1000])

        all_fitness = np.array(gen_book)
        all_max_fitness = np.array(gen_book_max)
        ...
        plt.plot(avg_of_max_fitness, label=f'{sub_label}', linewidth=0.5)
        plt.fill_between(range(len(avg_of_max_fitness)), avg_of_max_fitness - std_dev_max_fitness, avg_of_max_fitness + std_dev_max_fitness, alpha=0.2)
